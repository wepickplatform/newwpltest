...
export async function getPopularPosts() {
  try {
    // 최신 게시물 10개 가져오기 (per_page=10)
    const posts = await fetchAPI('/posts?per_page=10&orderby=date&order=desc');
    
    // 인기 게시물 형식으로 변환하여 의도된 SQL 쿼리 결과처럼 보이게 함
    const formattedPosts = posts.map((post, index) => ({
      ...
      // featured_image 필드가 없으므로 null로 설정
      featured_image: null
    }));
